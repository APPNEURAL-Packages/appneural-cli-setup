...
export function registerSetupCommands(program) {
    ...
    setup
        ...
        .action((role) => withTelemetry("setup:role", async () => {
    ...
    setup
        ...
        .action(() => withTelemetry("setup:local", async () => {
        const summary = await withSpinner("Bootstrapping APPNEURAL local environment", runLocalEnvironmentSetup, "ignite");
        ...
        logger.info(`APPNEURAL migrations: ${summary.migrationsExecuted ? "executed" : "skipped"}, seeders: ${summary.seedersExecuted ? "executed" : "skipped"}`);
        summary.healthChecks.forEach((check) => {
            logger.info(`APPNEURAL health ${check.url} => ${check.healthy ? "healthy" : "unavailable"}${check.status ? ` (status ${check.status})` : ""}`);
        });
