...
async function readAppneuralConfig() {
...
async function writeAppneuralConfig(config) {
    await fs.writeFile(CONFIG_FILE, JSON.stringify(config, null, 2), "utf-8");
}
async function loadRolesManifest() {
...
async function copyTemplateToGlobal(role, templateKey) {
...
async function copySnippetToGlobal(snippetKey) {
...
export async function applyRoleSetup(role) {
    ...
    const nextConfig = {
        ...
        healthChecks: definition.config?.healthChecks ?? currentConfig.healthChecks,
    ...
    (definition.shortcuts ?? []).forEach((shortcut) => {
        logger.info(`APPNEURAL shortcut '${shortcut.name}' => ${shortcut.command}${shortcut.description ? ` (${shortcut.description})` : ""}`);
    });
    return {
        role,
        shortcuts: definition.shortcuts ?? [],
...
function createSecret(length = 32) {
...
async function runScriptIfPresent(manager, script) {
    try {
        if (manager === "pnpm") {
...
async function startDockerServices(composeCommand) {
